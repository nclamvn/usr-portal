...
CSS = """
  ...
  /* INFO/02 homepage world signature — FULL-BLEED band (body has no padding, so width:100% = viewport) */
  .home-world{margin:32px 0 0;position:relative;border-top:1px solid var(--hair);border-bottom:1px solid var(--hair)}
  .home-world .wm-svg{max-height:none;width:100%}
  .home-world-more{display:block;text-align:right;font-family:var(--font-mono);font-size:.74rem;
    letter-spacing:.04em;color:var(--brass);text-decoration:none;max-width:var(--w-wide);
    margin:0 auto;padding:.7rem 1.4rem;opacity:.85}
  .home-world-more:hover{opacity:1}
  @media(max-width:760px){ .home-world{margin-top:24px} }
