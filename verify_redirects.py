...
"""PRD §4.2 URL pattern + §8.1.4 (link không vỡ) — giữ align /uav/ bền, fail-loud exit 2:
  · REDIRECT_MISSING — vercel.json thiếu 301 /entity/* -> /uav/* (link cũ đang index sẽ vỡ).
...
def main():
    ...
    ok = any("/entity/" in r.get("source", "") and "/uav/" in r.get("destination", "") and r.get("permanent") is True
             for r in reds)
    if not ok:
        fails.append("REDIRECT_MISSING: vercel.json không có 301 /entity/* -> /uav/*")
